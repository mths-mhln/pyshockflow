...
for prop_name in properties:
    ...
    # Compute gradients w.r.t. normalized S and T
    dprop_dS = np.gradient(prop_grid, S_norm, axis=0)   # derivative along S (columns)
    dprop_dT = np.gradient(prop_grid, T_norm, axis=1)   # derivative along T (rows)
    
    total_derivatives[prop_name] = dprop_dS + dprop_dT
    prop_vals[prop_name] = prop_grid
    ...
    if first_derivative:
        scatter = axes[0].scatter(
            S_grid,
            T_grid,
            c=total_derivatives[prop_name],
            cmap='viridis',
            s=5,
            zorder=0,
        )
        fig.colorbar(scatter, ax=axes[0], label=f'first derivative of {prop_name}')
...
#     T_range = config.thermoplot_settings["T_range"]
# else:
#     raise ValueError("Only TS diagram type is supported for this investigation.")
# # create grid of S, T pairs at which to evaluate thermodynamic properties
# n_pts = 10
# S = np.linspace(S_range[0], S_range[1], n_pts)
# T = np.linspace(T_range[0], T_range[1], n_pts)
# control_points = np.meshgrid(S, T)
# # print("control_points = ", control_points)

# # properties of interest to evaluate
# properties  = ["D", "U", "S", "P", "Q"]

# # The next steps aim to compute the total first derivative of the property on the 
# # normalized S and T grid. Normalization is necessary to obtain unbiased total derivative 
# # values. We are only interested in deviation from the neighbors.

# # normalize T and S coordinates
# S_normalized = (control_points[0] - S_range[0]) / (S_range[1] - S_range[0])
# T_normalized = (control_points[1] - T_range[0]) / (T_range[1] - T_range[0])

# # extract internal points 
# S_internal = S_normalized[1:-1, 1:-1]
# T_internal = T_normalized[1:-1, 1:-1]

# # instantiate total derivatives array
# total_derivatives = {}
# for property in properties:
#     total_derivatives[property] = np.zeros_like(S)

# for property in properties:
#     # compute the total derivatives of the property at the central control points with respect to S and T using central differences
#     S_left_central = S_normalized[1:-1, 0:-2]
#     S_right_central = S_normalized[1:-1, 2:]
#     T_bottom_central = T_normalized[0:-2, 1:-1]
#     T_top_central = T_normalized[2:, 1:-1]
#     partial_prop_S = (AS.PropsSI(property, "T", T_internal, "S", S_right_central) - AS.PropsSI(property, "T", T_internal, "S", S_left_central)) \
#                     / (2 * (S_right_central - S_left_central))
#     partial_prop_T = (AS.PropsSI(property, "T", T_top_central, "S", S_internal) - AS.PropsSI(property, "T", T_bottom_central, "S", S_internal)) \
#                     / (2 * (T_top_central - T_bottom_central))
#     total_derivatives[property][1:-1, 1:-1] = partial_prop_S + partial_prop_T

# idea: instead of making our lives hard and computing the derivatives at the edges as well (which can be done either by
# painstakingly computing the derivatives at the corners and edges, defining specialized routines for them, or by 
# extrapolating the edge values into "halo" nodes and applying the central differencing over the entire domain, which would
# return the derivatives over the full domain, albeit with slightly more shallow derivatives at the edges), we can simply 
# only look at the internal points and bump up the amount of control points in each direction. That saves quite some effort
# and still the majority of the insight we seek to obtain with this investigation. 
    ...
    # # compute the remaining total derivative of the property at the boundary points using forward and backward differences where necessary

    # # top left corner
    # partial_prop_S = (AS.PropsSI(property, "T", T_normalized[0, 0], "S", S_normalized[0, 1]) - AS.PropsSI(property, "T", T_normalized[0, 0], "S", S_normalized[0, 0])) \
    #                 / (2*(S_normalized[0, 1] - S_normalized[0, 0]))
    # partial_prop_T = (AS.PropsSI(property, "T", T_normalized[1, 0], "S", S_normalized[0, 0]) - AS.PropsSI(property, "T", T_normalized[0, 0], "S", S_normalized[0, 0])) \
    #                 / (2*(T_normalized[1, 0] - T_normalized[0, 0]))
    # total_derivatives[property][0, 0] = partial_prop_S + partial_prop_T

    # # top right corner
    # partial_prop_S = (AS.PropsSI(property, "T", T_normalized[0, -1], "S", S_normalized[0, -1]) - AS.PropsSI(property, "T", T_normalized[0, -1], "S", S_normalized[0, -2])) \
    #                 / (2*(S_normalized[0, -1] - S_normalized[0, -2]))
    # partial_prop_T = (AS.PropsSI(property, "T", T_normalized[1, -1], "S", S_normalized[0, -1]) - AS.PropsSI(property, "T", T_normalized[0, -1], "S", S_normalized[0, -1])) \
    #                 / (2*(T_normalized[1, -1] - T_normalized[0, -1]))
    # total_derivatives[property][0, -1] = partial_prop_S + partial_prop_T

    # # bottom left corner
    # partial_prop_S = (AS.PropsSI(property, "T", T_normalized[-1, 0], "S", S_normalized[-1, 1]) - AS.PropsSI(property, "T", T_normalized[-1, 0], "S", S_normalized[-1, 0])) \
    #                 / (2*(S_normalized[-1, 1] - S_normalized[-1, 0]))
    # partial_prop_T = (AS.PropsSI(property, "T", T_normalized[-2, 0], "S", S_normalized[-1, 0]) - AS.PropsSI(property, "T", T_normalized[-1, 0], "S", S_normalized[-1, 0])) \
    #                 / (2*(T_normalized[-2, 0] - T_normalized[-1, 0]))
    # total_derivatives[property][-1, 0] = partial_prop_S + partial_prop_T

    # # bottom right corner
    # partial_prop_S = (AS.PropsSI(property, "T", T_normalized[-1, -1], "S", S_normalized[-1, -1]) - AS.PropsSI(property, "T", T_normalized[-1, -1], "S", S_normalized[-1, -2])) \
    #                 / (2*(S_normalized[-1, -1] - S_normalized[-1, -2]))
    # partial_prop_T = (AS.PropsSI(property, "T", T_normalized[-2, -1], "S", S_normalized[-1, -1]) - AS.PropsSI(property, "T", T_normalized[-1, -1], "S", S_normalized[-1, -1])) \
    #                 / (2*(T_normalized[-2, -1] - T_normalized[-1, -1]))
    # total_derivatives[property][-1, -1] = partial_prop_S + partial_prop_T

    # # left edge
    # T_bottom = T_normalized[0:-2, 0]
    # T_top = T_normalized[2:, 0]
    # partial_prop_S = (AS.PropsSI(property, "T", T_normalized[1:-1, 0], "S", S_normalized[1:-1, 1]) - AS.PropsSI(property, "T", T_normalized[1:-1, 0], "S", S_normalized[1:-1, 0])) \
    #                 / (2*(S_normalized[1:-1, 1] - S_normalized[1:-1, 0]))
    # partial_prop_T = (AS.PropsSI(property, "T", T_top, "S", S_normalized[1:-1, 0]) - AS.PropsSI(property, "T", T_bottom, "S", S_normalized[1:-1, 0])) \
    #                 / (2*(T_top - T_bottom))
    # total_derivatives[property][1:-1, 0] = partial_prop_S + partial_prop_T
