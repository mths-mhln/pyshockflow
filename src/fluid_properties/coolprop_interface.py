...
class CoolPropFluid(base_interface.Fluid):
    ...
    def update_and_get(AS, input_spec, x, y, output, reorder):
        if reorder:
            AS.update(input_spec, y, x)
        else:
    ...
    def PropsSI(self, prop, x_str, x, y_str, y):
        ...
        if prop in ['Tcrit', 'Pcrit', 'Tmax', 'M']: # translation necessary to comply with AS syntax, see: https://coolprop.org/_static/doxygen/html/class_cool_prop_1_1_abstract_state.html
            if prop== 'Tcrit':
