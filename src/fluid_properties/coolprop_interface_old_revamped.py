...
class CoolPropAbstractState_v2():
    ...
    def _extract(AS: AbstractState, prop_AS: str, input_spec: int, x: float, y: float,
                    reorder: bool, out_key: str, x_str_AS: str, y_str_AS: str, verbose: bool = False):
        """
        ...
        if failed:
            val = CoolPropAbstractState_v2._try_critical_recovery(
                AS, x_str_AS, x, y_str_AS, y, out_key
            )
        else:
    ...
    def PropsSI(self, prop: str, x_str: str = None, x: float | np.ndarray = None, y_str: str = None, y: float | np.ndarray = None, verbose: bool = False) -> float | np.ndarray:
        ...
        # Translate PropsSI syntax to AbstractState syntax for the output property
        out_key = self._TRANSLATOR.get(prop_AS, prop_AS)

        return self._extract(AS, prop_AS, input_spec, x, y, reorder, out_key, x_str_AS, y_str_AS, verbose)


        # # evaluation for array inputs
